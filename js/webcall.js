			...
			openChat:function(c){
				...
				try{
					if(ev.min == 1 && jesong.force && jesong.force.openchat){
						/*if(clickText != "" && jesong.force.status == 2){
							jesong.force.sendMessage(clickText);
						}else{
							if(clickText != ""){
								jesong.autochat.waitSendMsg = clickText;
							}
					...
					img.src= ev.server.monitor + "i?cmd=clickError&c="+ev.compId+'&v='+ev.vId+'&u='+ev.uId + "&j="+ev.confId
						+"clickText=" + window.encodeURIComponent(clickText)
					...
					//var url = "" + "url="+window.encodeURIComponent(window.location.href) + ;
				}
